...
namespace EstoqueWeb.Controllers
{
    public class ProdutoController : Controller
    {
            ...
            if (ModelState.IsValid)
            {
                if (id.HasValue)
                {
                    if (ProdutoExiste(id.Value))
                    {
                        ...
                        {
                            TempData["mensagem"] = MensagemModel.Serializar("Erro ao alterar produto.", MensagemTipo.Erro);
                        }
                    }
                    else
                    {
                        TempData["mensagem"] = MensagemModel.Serializar("Produto não encontrado.", MensagemTipo.Erro);
                    }
                }
                else
                {
                    ...
                    {
                        TempData["mensagem"] = MensagemModel.Serializar("Erro ao cadastrar produto.", MensagemTipo.Erro);
                    }
                }
        ...
        public async Task<IActionResult> Excluir(int? id)
        {
            if (!id.HasValue)
            {
                TempData["mensagem"] = MensagemModel.Serializar("Produto não informado.", MensagemTipo.Erro);
                return RedirectToAction(nameof(Index));
            }
        ...
        public async Task<IActionResult> Excluir(int id)
        {
            ...
            if (produto != null)
            {
                ...
                if (await _context.SaveChangesAsync() > 0)
                    ...
                    TempData["mensagem"] = MensagemModel.Serializar("Não foi possível excluir o produto.", MensagemTipo.Erro);                
            }
            else
            {
                TempData["mensagem"] = MensagemModel.Serializar("Produto não encontrado.", MensagemTipo.Erro);
